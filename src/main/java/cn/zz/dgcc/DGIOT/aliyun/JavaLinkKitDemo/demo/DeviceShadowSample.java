package cn.zz.dgcc.DGIOT.aliyun.JavaLinkKitDemo.demo;
...
public class DeviceShadowSample {
    ...
    private final String shadowUpdate = "{" + "\"method\": \"update\"," + "\"state\": {" + "\"reported\": {" +
            "\"color\": \"red\"" + ",\"mode\": \"1\"" + "}" + "}," + "\"version\": {ver}" + "}";

    // 获取设备影子
    private final String shadowGet = "{" + "\"method\": \"get\"" + "}";

    //删除设备影子 color 属性  {ver}需要替换
    private final String shadowDelete = "{" + "\"method\": \"delete\"," + "\"state\": {" + "\"reported\": {" +
            "\"color\": \"null\"" + "}" + "}," + "\"version\": {ver}" + "}";

    //删除设备影子所有属性  {ver}需要替换
    private final String shadowDeleteAll = "{" + "\"method\": \"delete\"," + "\"state\": {" +
                        ...
                        try {
                            if (aRequest != null) {
                                ...
                                if (aResponse.data instanceof byte[]) {
                                    dataStr = new String((byte[]) aResponse.data, StandardCharsets.UTF_8);
