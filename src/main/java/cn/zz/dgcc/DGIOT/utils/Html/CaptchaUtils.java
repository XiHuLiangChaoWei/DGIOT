...
public class CaptchaUtils {
     ...
     * 生成一个 count位的验证码
     */
    public static String CaptchaStr(int count){
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < count; i++){
            char aChar = chars[(int)(Math.random() * chars.length)];
            sb.append(aChar);
        }
        System.err.println("CaptchaUtils.CaptchaStr:"+sb);
        return sb.toString();
    }
    ...
    public static BufferedImage genCaptchaImg(String captcha){
        System.err.println("CaptchaUtils.generateImage：start");
        ThreadLocalRandom r = ThreadLocalRandom.current();

        int count = captcha.length();
        int fontSize = 100; //code的字体大小
        int fontMargin = fontSize/6; //字符间隔
        int width = (fontSize)*count+fontMargin; //图片长度
        ...
        //验证码的颜色
        Color[] catchaColor = {Color.BLACK, Color.BLUE, Color.RED, Color.GREEN};
        ...
        g.dispose();
        System.err.println("CaptchaUtils.generateImage: complete！");
        return image;
    }
     ...
     * 生成验证码并保存到session中
     */
    public static BufferedImage genCaptcha(HttpServletRequest request, HttpServletResponse response) {

        //生成一个校验码
        String captcha = CaptchaUtils.CaptchaStr(5);

        //生成一个cookie ID，并塞进response里面
        String cookieId = UUID.randomUUID().toString().replace("-", "").toUpperCase();
        //创建一个cookie, 没有设置失效时间, 默认关闭浏览器失效
        Cookie cookie = new Cookie("YsbCaptcha", cookieId);
        //设置cookie失效时间, 默认两分钟
        cookie.setMaxAge(CodeTypeUtils.CAPTCHA_EXPIRED * 60);
        response.addCookie(cookie);

        //把校验码、是否已经通过校验（缺省不设置为0）保存到session中，以cookie ID 为key
        HttpSession session = request.getSession();
        //将cookieId 和 验证码值设置进session中
        session.setAttribute(CodeTypeUtils.CAPTCHA+cookieId,captcha);
        //设置session失效时间, 默认两分钟
        session.setMaxInactiveInterval(CodeTypeUtils.CAPTCHA_EXPIRED * 60);
